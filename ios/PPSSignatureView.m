...
static BOOL isSignatureEmpty(UIImage *myImage)
{
    typedef struct
    {
        uint8_t red;
    ...
    size_t pixelCount = CGImageGetWidth(myCGImage) * CGImageGetHeight(myCGImage);
		...
		// Count non empty pixel count
    uint nonEmptyPixelCount = 0;
    ...
    float nonEmptyPixelPercentage = nonEmptyPixelCount * 100 / (float) pixelCount;
    
		// Treat signature as empty if it's below 0.5% content threshold
    return nonEmptyPixelPercentage < 0.5;
}
...
- (UIImage *)signatureImage: (BOOL) rotatedImage withSquare:(BOOL) square
{
    ...
    if (isSignatureEmpty(signatureImg)) {
        return nil;
    }
